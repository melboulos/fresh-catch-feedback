...
Known from the Fresh Catch side: Pursue owns deep person research, LinkedIn history mining, signal-progression tracking, Couchbase customer-reference research (evidence-gated: `DIRECT_MATCH` / `PARTIAL_MATCH` / `NO_MATCH` + one-example-max), and email drafting from persisted lead context — triggered when a rep clicks the 🎯 Pursue pill in a Fresh Catch email.
